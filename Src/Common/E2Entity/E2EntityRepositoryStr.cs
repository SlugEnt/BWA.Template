...
public class E2EntityRepositoryStr<TEntityStr> : E2EntityRepository<TEntityStr>, IDisposable, IEntityRepositoryE2Str<TEntityStr>
    where TEntityStr : AbstractEntityStr, IEntityStr, new()
{
    ...
    public E2EntityRepositoryStr(AppDbContext db,
                                    ILogger<E2EntityRepositoryStr<TEntityStr>> logger) : base(db, logger)
    { }
    ...
    /// <returns>Result.Ok::TEntityStr if success.  Failed if errors or record not found.</returns>
    protected async Task<Result<TEntityStr?>> GetByIdAsync(string id,
                                                           bool respectActiveFlag)
    {
        try
        {
            var qry = _db.Set<TEntityStr>().Where(e => e.Id == id);
            if (respectActiveFlag)
                qry = qry.Where(e => e.IsActive == true);
            TEntityStr? entity = await qry.FirstOrDefaultAsync();
            if (entity == null)
                return Result.Fail("Unable to find " + typeof(TEntityStr).Name + " with Id: " + id);
                                              ...
                                              int updatedByPersonId)
    {
        ...
        {
            // Lookup the Entity.
            Result<TEntityStr?> entityResult = await GetByIdAsync(id, false);
            ...
            TEntityStr entity = entityResult.Value;
            if (entity == null)
            {
                                          ...
                                          int updatedByPersonId)
    {
        try
        {
            ...
            Result<TEntityStr?> entityResult = await GetByIdAsync(id, false); // We want to check if it exists regardless of active status
            ...
            _db.Set<TEntityStr>().Remove(entityResult.Value);
                                            ...
                                            int updatedByPersonId)
    {
        try
        {
            ...
            if (entity.IsActive != true)
            {
                ...
                _db.Set<TEntityStr>().Update(entity);
                await _db.SaveChangesAsync();
                return Result.Ok();
            }
