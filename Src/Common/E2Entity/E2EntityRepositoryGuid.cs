...
public class E2EntityRepositoryGuid<TEntityGuid> : E2EntityRepository<TEntityGuid>, IDisposable, IEntityRepositoryE2Guid<TEntityGuid>
    where TEntityGuid :  AbstractEntityGuid, IEntityGuid, new()
{
    ...
    public E2EntityRepositoryGuid(AppDbContext db,
                                 ILogger<E2EntityRepositoryGuid<TEntityGuid>> logger) : base(db, logger) 
    { }
    ...
    /// <returns>Result.Ok::TEntityInt if success.  Failed if errors or record not found.</returns>
    protected async Task<Result<TEntityGuid?>> GetByIdAsync(Guid id,
                                                           bool respectActiveFlag)
    {
        try
        {
            var qry = _db.Set<TEntityGuid>().Where(e => e.Id == id);
            if (respectActiveFlag)
                qry = qry.Where(e => e.IsActive == true);
            TEntityGuid? entity = await qry.FirstOrDefaultAsync();
            if (entity == null)
                return Result.Fail("Unable to find " + typeof(TEntityGuid).Name + " with Id: " + id);
                                              ...
                                              int updatedByPersonId)
    {
        ...
        {
            // Lookup the Entity.
            Result<TEntityGuid?> entityResult = await GetByIdAsync(id, false);
            ...
            TEntityGuid entity = entityResult.Value;
            if (entity == null)
            {
                                          ...
                                          int updatedByPersonId)
    {
        try
        {
            ...
            Result<TEntityGuid?> entityResult = await GetByIdAsync(id, false); // We want to check if it exists regardless of active status
            ...
            _db.Set<TEntityGuid>().Remove(entityResult.Value);
                                            ...
                                            int updatedByPersonId)
    {
        try
        {
            ...
            if (entity.IsActive != true)
            {
                ...
                _db.Set<TEntityGuid>().Update(entity);
                await _db.SaveChangesAsync();
                return Result.Ok();
            }
