...
public class E2EntityRepositoryLong<TEntityLong> : E2EntityRepository<TEntityLong>, IDisposable, IEntityRepositoryE2Long<TEntityLong>
    where TEntityLong :  AbstractEntityLong, IEntityLong, new()
{
    ...
    public E2EntityRepositoryLong(AppDbContext db,
                                 ILogger<E2EntityRepositoryLong<TEntityLong>> logger) : base(db, logger) 
    { }
    ...
    public async Task<Result<TEntityLong?>> GetByIdAsync(long id)
    {
    ...
    /// <returns>Result.Ok::TEntityInt if success.  Failed if errors or record not found.</returns>
    /// <exception cref="NotImplementedException"></exception>
    public async Task<Result<TEntityLong?>> GetByIdAnyStatusAsync(long id)
    {
    ...
    /// <returns>Result.Ok::TEntityInt if success.  Failed if errors or record not found.</returns>
    protected async Task<Result<TEntityLong?>> GetByIdAsync(long id,
                                                           bool respectActiveFlag)
    {
        try
        {
            var qry = _db.Set<TEntityLong>().Where(e => e.Id == id);
            if (respectActiveFlag)
                qry = qry.Where(e => e.IsActive == true);
            TEntityLong? entity = await qry.FirstOrDefaultAsync();
            if (entity == null)
                return Result.Fail("Unable to find " + typeof(TEntityLong).Name + " with Id: " + id);
                                              ...
                                              int updatedByPersonId)
    {
        ...
        {
            // Lookup the Entity.
            Result<TEntityLong?> entityResult = await GetByIdAsync(id, false);
            ...
            TEntityLong entity = entityResult.Value;
            if (entity == null)
            {
                                          ...
                                          int updatedByPersonId)
    {
        try
        {
            ...
            Result<TEntityLong?> entityResult = await GetByIdAsync(id, false); // We want to check if it exists regardless of active status
            ...
            _db.Set<TEntityLong>().Remove(entityResult.Value);
    ...
    /// <param name="updatedByPersonId"></param>
    /// <returns></returns>
    public async Task<Result> ActivateAsync(long id,
                                            int updatedByPersonId)
    {
        try
        {
            ...
            if (entity.IsActive != true)
            {
                ...
                _db.Set<TEntityLong>().Update(entity);
                await _db.SaveChangesAsync();
                return Result.Ok();
            }
