...
/// <summary>
/// Provides access for an Entity based upon an UliD identifier.
/// </summary>
/// <typeparam name="TEntityUlid"></typeparam>
public class E2EntityRepositoryULid<TEntityUlid> : E2EntityRepository<TEntityUlid>, IDisposable, IEntityRepositoryE2Ulid<TEntityUlid>
    where TEntityUlid : AbstractEntityULID, IEntityULID, new()
{
    ...
    public E2EntityRepositoryULid(AppDbContext db,
                                    ILogger<E2EntityRepositoryULid<TEntityUlid>> logger) : base(db, logger)
    { }
    ...
    /// </summary>
    /// <param name="id"></param>
    /// <returns>Result.Ok::TEntityStr if success.  Failed if errors or record not found.</returns>
    ...
    /// <returns>Result.Ok::TEntityStr if success.  Failed if errors or record not found.</returns>
    protected async Task<Result<TEntityUlid?>> GetByIdAsync(Ulid id,
                                                           bool respectActiveFlag)
    {
        try
        {
            var qry = _db.Set<TEntityUlid>().Where(e => e.Id == id);
            if (respectActiveFlag)
                qry = qry.Where(e => e.IsActive == true);
            TEntityUlid? entity = await qry.FirstOrDefaultAsync();
            if (entity == null)
                return Result.Fail("Unable to find " + typeof(TEntityUlid).Name + " with Id: " + id);
                                              ...
                                              int updatedByPersonId)
    {
        ...
        {
            // Lookup the Entity.
            Result<TEntityUlid?> entityResult = await GetByIdAsync(id, false);
            ...
            TEntityUlid entity = entityResult.Value;
            if (entity == null)
            {
                                          ...
                                          int updatedByPersonId)
    {
        try
        {
            ...
            Result<TEntityUlid?> entityResult = await GetByIdAsync(id, false); // We want to check if it exists regardless of active status
            ...
            _db.Set<TEntityUlid>().Remove(entityResult.Value);
            await _db.SaveChangesAsync();

            // This was not removing it from the local DB store.... Not worth the work around at this time.
            // int rowsDeleted =  await _db.Set<TEntityStr>().Where(e => e.Id == id).ExecuteDeleteAsync();
            return Result.Ok();
        }
        catch (Exception e)
        {
                                            ...
                                            int updatedByPersonId)
    {
        try
        {
            ...
            if (entity.IsActive != true)
            {
                ...
                _db.Set<TEntityUlid>().Update(entity);
                await _db.SaveChangesAsync();
                return Result.Ok();
            }
