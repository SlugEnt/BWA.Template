...
public class SampleLongFluentValidator : AbstractValidator<SampleLong>
{
    public SampleLongFluentValidator()
    {
    ...
    public Func<object, string, Task<IEnumerable<string>>> ValidateValue =>
        async (model,
               propertyName) =>
        {
            var result = await ValidateAsync(ValidationContext<SampleLong>.CreateWithOptions((SampleLong)model, x => x.IncludeProperties(propertyName)));
            if (result.IsValid)
